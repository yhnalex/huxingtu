...
import xuanzhuanjiaozheng as xz
import requests
import json
import base64
import qiangtishibie
...
request_url = "https://aip.baidubce.com/rpc/2.0/ai_custom/v1/detection/get_outside"
request_url = request_url + "?access_token=" + access_token


image_path = 'D:/huxingku_train/938783fd68d374fe68782480c2868123.jpg'
# file = open(image_path, 'rb')
img = xz.get_rotate_img(image_path)
data = {"image": image_to_base64(img)}
res2 = requests.post(request_url, data=json.dumps(data), headers=headers)
content_dict = json.loads(res2.text)
print (content_dict)
...
for item in content_dict['results']:
    if item['name'] == 'huxingtu' and item['score'] > score:
        shape = [item['location']['height'], item['location']['left'], item['location']['top'], item['location']['width']]
...
cropImg = img[(height - shape[2] - shape[0]):(height - shape[2]), shape[1]:(shape[1]+shape[3])]
cropImg_withoutcolor = xz.ruihua(xz.mohu(xz.quzao(cv2.cvtColor(cropImg,cv2.COLOR_BGR2GRAY))))
cv2.imwrite('D:/huxingku_train/test_.jpg',cropImg_withoutcolor)
img_qiangti = qiangtishibie.qiangtishibie(cropImg_withoutcolor)

cv2.imshow('img', img_qiangti)
cv2.waitKey(0)
